...
### 16.5 후속 회귀 — 수임처명 대괄호가 glob 문자 클래스로 해석 (2026-08-19)

16.2 의 `_resolve_insurance_dir` 로 **디렉토리**는 정상 리졸브되는데, 그 안에서
건강보험 PDF 만 못 찾는 별개 회귀. 위하고 수임처명에는 담당자 접두가 붙고
(`[손예린] 엘리야레포츠주식회사`) 그대로 폴더명이 되는데, **glob 은 `[손예린]` 을
리터럴이 아니라 '손·예·린 중 한 글자' 문자 클래스로 해석**한다 → 경로 불일치 →
매칭 0건.

- **건강보험만 터진 이유**: 같은 함수 안에서 NPS·고용보험은 `os.listdir()` + 문자열
  `in` 검사인데 **건강보험만 `glob.glob()`** 을 썼다(`wehago_swsa.py:270`).
- **조용한 실패**: `nhis_pdf=None` 이면 예외·경고 없이 병합만 스킵된다. 다운로드도
  업로드도 성공하므로 **건강보험/장기요양/정산 컬럼만 0원**인 채 넘어간다.
  실측: 엘리야레포츠 202608 — PDF 27명 파싱 정상인데 업로드 엑셀 전원 0원,
  같은 파일의 국민연금 컬럼은 정상 반영(비대칭이 진단의 결정적 단서).
- **동일 결함 2번째 지점**: `hometax.py:52` 의 `.01` 신고파일 탐색(`save_dir` 에도
  수임처명이 그대로 들어간다). 이쪽은 `state.fail_step` 으로 명시적 실패라 조용하지는
  않지만, 원인이 '파일 없음' 으로 보고돼 오진하기 쉽다.
- **수정**: 두 곳 모두 **디렉토리 부분에만** `glob.escape()` 적용(패턴의 `*` 는 보존).
- **회귀 테스트**: `tests/test_bracket_client_name_glob.py` — 대괄호/평범한 이름을
  parametrize 로 대조하고, 두 경우의 탐색 결과 집합이 같은지까지 확인한다.
- ★**교훈**: 사용자 데이터(수임처명)가 경로에 들어가는 곳에서 `glob` 을 쓸 때는
  디렉토리를 반드시 이스케이프한다. 34건 중 5건이 대괄호 수임처였고, 이 5건만
  조용히 틀린 값으로 업로드됐다.
