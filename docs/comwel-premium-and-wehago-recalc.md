...
## 5. 단독 고용보험 선택건 재실행 복구 (`v1.1.12`)

### 증상과 원인

단독 고용보험 EDI에서 한 수임처를 완료한 뒤 같은 GUI 세션으로 다시 선택건 실행하면,
Chrome 9223의 기존 20209 SPA 탭을 재사용한다. 기존 판정은 관리번호 입력란이 보이면
20209 화면이 준비됐다고 보아 재진입을 생략했다. 하지만 WebSquare 화면 상태가 남은 경우
`사업장조회` 버튼 DOM은 존재해도 WZ0101_P01 팝업이 열리지 않아 실제 브라우저 동작 없이
전환이 실패할 수 있었다.

### 복구 동작

`ComwelEdiWorkflow.run_single()`은 최초 사업장 전환 실패에만 다음을 한 번 수행한다.

1. `reset_workplace_page()`로 COMWEL 메인 화면을 연다. 이 헬퍼는 성공 여부를 `bool`로
   반환하고 실패는 로그만 남긴다.
2. 20209 화면을 다시 열고 GUI에서 지정한 연·월을 다시 설정한다.
3. 같은 관리번호·수임처로 사업장 전환을 한 번 재시도한다.

재시도까지 실패하거나 복구 중 예외가 나면 해당 건만 실패 처리하고 메인 화면을 다시 열어
다음 선택건 실행이 이전 SPA 상태에 묶이지 않게 한다. Chrome·공동인증서 로그인 세션은
종료하지 않으며, 병렬 EDI의 고용보험 9225 전용 프로필/CLI 경로는 변경하지 않는다.

### 검증

- 회귀 테스트: 최초 실패 후 복구 성공, 재시도 최종 실패 뒤 리셋, 복구 중 재진입 예외,
  최초 성공 경로의 무복구를 `tests/test_comwel_single_recovery.py`로 고정.
- 라이브 GUI 검증(2026-08-11): 단독 고용보험에서 한 선택건 완료 후 다른 수임처 선택건을
  같은 GUI 세션에서 실행해 정상 진행을 확인.
