...
| 2026-07-29 | `5198a37` | **Phase 7 급여 PDF 0건 버그 수정(부채 항목 아님 · 외부 UI 회귀).** WEHAGO 가 `#print→일괄출력` 메뉴와 Duzon PrintDialog 사이에 웹 모달을 삽입 → 모달 내 `[일괄출력]` 클릭 단계 부재로 항상 타임아웃. 개발 PC에서도 100% 재현(특정 PC 문제 아님). 함께 수정: ①회차 그리드 체크 선행 ②지급일 확보(귀속연월만으론 조회 미완성) ③**PDF 0건을 `False`+`fail_step` 으로 보고** — 구 구현은 0건인데 `True` 를 반환하는 **무증상 실패**였다. 회귀 테스트 18종 신설, 전체 278 passed. 라이브(리틀치프 2026-03) + **GUI 실행** 모두 검증. ⚠**교훈:** 산출물 0건을 성공으로 보고하는 경로가 다른 Phase 에도 있는지 점검 필요(잠재 부채) |
