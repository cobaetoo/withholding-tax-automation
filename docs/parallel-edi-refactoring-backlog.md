...
|------|-----|
| **작성일** | 2026-08-06 |
| **재검증** | 2026-08-19 — v1.1.12 / `f99ee68` 코드 대조(PR #13 포함). **PE-P0-5 ✅**. 나머지 VALID. 신규 PE 없음 |
| **기준 버전** | `1.1.12` (`src/version.py`) |
| **기준 커밋** | `f99ee68` (재검증 HEAD, PR #13 포함). 안정화 원점 `deb0791` (v1.1.4) |
...
코드(`1.1.12` / `f99ee68`)와 전 항목을 다시 대조했다. 08-07 이후 기능 커밋: `9c34613`(NHIS 탭·전환 실패 전파), `afd63c9`(병렬 사전점검), `47f3722`(NPS 사업장 누락 시 계속), `d68b41a`(COMWEL 팝업 복구), PR #13(NPS 검색 콤보·수임처명 경로·WEHAGO F4 폴백). **구조 리팩터는 없음.**
...
| 2026-08-19 | 재검증(v1.1.12, `f99ee68`, PR #13 포함). PE-P0-5 ✅ · §1.1 계약 3건 추가 · Wave C에서 P0-5 제외 · P0-3/P1-4 문구 정밀화. 신규 PE 없음. **코드 리팩터 미실시.** |
