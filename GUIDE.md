...
| 12 | 홈택스 | 원천세 신고 | 완료 |
| 13 | 위택스 | 지방세 신고 (특별징수·회계파일신고) | 부분 (로그인~파일비번, 업로드 대기) |
...
**상태: 부분 구현 (2026-07-24)** — 로그인~파일비밀번호까지 라이브/GUI 검증.  
**암호화 파일선택·파일변환·제출은 업로드 파일 준비 후 구현.** 상세: `src/automation/wetax/PROGRESS.md`.
...
| 5 | 암호화 파일선택 | Phase 11 전자신고파일 · `#file_upload_0_` | ⏸ 대기 |
| 6 | 파일변환하기 | `#btn_next` | ⏸ 대기 |
| 7 | 제출하기 | **2 서식검증 및 제출** 탭 이후 (+ dry-run 예정) | ⏸ 대기 |

**GUI**
- 사이드바: 「위택스」 > 「위택스 지방세 신고」(phase 13)
- 툴바: **비밀번호** (`needs_password`) + **휴대전화** (`needs_phone`)
