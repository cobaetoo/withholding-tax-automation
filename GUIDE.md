...
세무법인에서 사용하는 원천징수 업무 자동화 Windows 데스크톱 애플리케이션. 5개 포털(WEHAGO/NHIS EDI/NPS EDI/홈택스/위택스)에 걸친 13개 Phase 워크플로우를 하나의 GUI에서 관리하고, 수임처별 진행 상황을 실시간으로 추적한다.
...
| 2 | (병렬) | 공단 EDI 병렬 자동화 (NPS+NHIS+고용보험) | 완료 (v1.1.5 NHIS 전환·실패 전파 보강) |
| 3 | 국민건강보험 EDI | 결정내역 PDF/Excel 다운로드 | 완료 (v1.1.6 받은문서 탭 감지 보강) |
...
7. **원천징수 신고주기 태그 추출:** 각 카드의 태그(`div.tag_bx.is_tag button.btn_tag span`)에서 `매월`/`반기`를 찾아 `report_cycle`로 수집. **2단 판별** — ①완전일치: 콤마 세그먼트가 정확히 `매월`/`반기`(예: `테스트1,원천,매월`). ②부분일치 폴백(①실패 시에만): 태그가 중첩 엘리먼트로 렌더돼 구분자 없이 이어붙은 경우(예: `한정선원천반기`)를 구제. 단 매월·반기가 **동시 관측되면 추측하지 않고 미지정**으로 남긴다(실행 시 화면 ground truth 로 재판별). ②로 잡힌 건은 `WARN 신고주기 부분일치` 로그에 수임처명·태그가 남는다
...
> **v1.1.6 단일 실행 안정화:** 받은문서 클릭 직후 웹EDI 탭이 동기적으로 열리는 환경에서도 탭을 감지하도록, 클릭 **전** 탭 목록을 기준으로 새 탭을 판정한다. 기존에는 클릭 뒤에 탭 목록을 기록해 즉시 열린 탭을 기존 탭으로 오인하고 `새 탭 미감지`로 실패할 수 있었다.
...
### Phase 10: 지방소득세특별징수납부서 (WEHAGO — SWTA0112)

원천이행상황신고서(SWTA0101)의 **지방세 짝**. SmartA 안에서 SWTA0112 메뉴로 진입해 동일한 방식(**마감**)으로 처리한다. 매월·반기 모두 지원. 비밀번호 불필요.

**상태: 라이브 검증 완료 (2026-07)** — 리틀치프코리아(매월)·끽비어컴퍼니(반기). pytest 178 통과.

1. Chrome 실행 → WEHAGO 접속 (Phase 1 등 다른 WEHAGO phase와 포털 공유)
2. 사용자 공동인증서 수동 로그인
3. 수임처별 순차 처리:
   - **WEHAGO 메인 복귀** → 사업자번호 검색 → 수임처명 fallback
   - **SWSA0101 사이드바 클릭** (SPA 라우팅 초기화) → **SWTA0112 메뉴 이동** → 신고주기 결정
   - **신고주기 결정**: DB `clients.report_cycle`(매월/반기) 우선. 원천이행상황신고서(SWTA0101)와 동일한 판별 로직 재사용.
   - **기간 설정**:
     - **매월** → GUI 선택 연/월 (미선택 시 직전월 자동 계산)
     - **반기** → 유저 입력월 기준(`compute_half_period()`). 반기 신고는 연 2회이며 **6·12월에만 실행**(그 외 월은 마감 스킵).
   - **조회 → 마감/마감해제**: 미마감이면 마감 적용(2단계 모달). 이미 마감이면 **마감해제 → (같은 월) 재조회 → 재마감** 사이클로 동작 — 원천이행상황신고서(SWTA0101)와 동일 패턴.

### Phase 11: 지방소득세특별징수전자신고 (WEHAGO — SWER0109)

원천전자신고(SWER0101)의 **지방세 짝**. SmartA 안에서 SWER0109 메뉴로 진입해 동일한 방식으로 전자신고 파일을 제작한다. 전자신고 파일 비밀번호와 **WehagoNTS 프로그램 설치**가 필요.

**상태: 라이브 검증 완료 (2026-07)** — 리틀치프코리아(매월)·끽비어컴퍼니(반기). pytest 178 통과.

> **★전제조건:** 반드시 **지방소득세특별징수납부서(Phase 10, SWTA0112)를 먼저 마감**해야 전자신고 데이터가 생성된다. 마감하지 않으면 수임처 조회 시 "조회할 데이터 없음"이 되어 파일이 만들어지지 않는다. (원천세의 "원천이행상황 마감 → 원천전자신고 제작"과 동일한 순서 의존성.)

1. Chrome 실행 → WEHAGO 접속 (Phase 1 등 다른 WEHAGO phase와 포털 공유)
2. 사용자 공동인증서 수동 로그인
3. **사전 요구: WehagoNTS 프로그램 설치** — 전자신고 파일 저장 시 COM UIAutomation으로 WehagoNTS 제어. 미설치 시 파일 저장 단계에서 실패.
4. **"전체실행" 클릭 시 툴바 비밀번호 필드에서 읽기** (전자신고 phase 선택 시 툴바에 비밀번호 입력란 표시)
5. 수임처별 순차 처리:
   - **WEHAGO 메인 복귀** → 사업자번호 검색 → 수임처명 fallback
   - **SWSA0101 사이드바 클릭** (SPA 라우팅 초기화) → **SWER0109 이동**
   - **지급기간 설정**: GUI 연도/월 반영 (미선택 시 전월 자동 계산)
   - **수임처 선택** → 코드도움 확인 모달 처리
   - **제작(F4)** → 참고사항 모달 닫기 → 변환파일 비밀번호 입력 → 전자신고 파일 제작
   - **WehagoNTS 폴더 선택**: COM UIAutomation으로 지방소득세 특별징수 전자신고 파일 저장
...
### Phase 13: 위택스 지방세 신고 (특별징수세액 · 회계파일신고)

WEHAGO Phase 11(지방소득세특별징수전자신고, SWER0109)에서 생성된 **전자신고 파일**을
...
| Phase | 포털 | URL |
|-------|------|-----|
| 1, 6, 7, 8, 9, 10, 11 | WEHAGO | https://www.wehago.com/ |
| 3 | 국민건강보험 EDI | https://edi.nhis.or.kr/ |
| 4 | 국민연금 EDI | https://edi.nps.or.kr/ |
...
고용보험 단독 EDI는 동일 GUI 세션에서 이전 선택건의 20209 SPA 탭을 재사용한다.
v1.1.12부터 사업장조회 팝업이 열리지 않아 사업장 전환에 실패하면 로그인 세션을 유지한 채
메인 화면 → 20209 재진입 → 연월 재설정 후 같은 수임처를 **한 번만** 재시도한다. 재시도도
실패하면 해당 건만 실패로 남기고 화면을 리셋해 다음 선택건 실행을 준비한다. 병렬 EDI의
고용보험 전용 Chrome(9225) 경로는 이 복구 대상이 아니다.
